...
Among them, \citet{ahmadian2021meta} provide the most comprehensive baseline by evaluating all problem scales, whereas other recent works \citep{abderrazzak2022adaptive, sabri2023reinforcement} focus on small and medium instances.
...
Most published methods rely either on random, unfiltered operation moves \citep{dos2010combination, ahmadian2021meta} or on local search operators adapted from makespan minimization that ignore operation due dates.
...
\begin{enumerate}[(i)]
    ...
    \item We establish an AAD validation framework parameterized via a context-free BNF grammar to evaluate the proposed operator objectively among diverse algorithmic components.
    \item We conduct extensive computational experiments on the 72 standard benchmark instances of \citet{baptiste2008lagrangian}, showing that \texttt{irace} selects \texttt{swap\_crit} in 80\% of elite configurations, with the resulting algorithm matching or improving upon state-of-the-art literature results across 72.2\% of the instances and establishing 25 new best-known solutions.
\end{enumerate}

The remainder of this paper is organized as follows:
Subsections~\ref{subsec:prob} and \ref{subsec:litrev} formulate the JITJSS problem and review the related literature, respectively.
