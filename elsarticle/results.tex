...
While all runs exhausted their allocated budgets, configuration $C_1$ converged to its final best solutions in an average of 3.7, 20.7 and 61.4 seconds for 10, 15 and 20 jobs instances, demonstrating that high-quality schedules are identified well before the conservative time horizons expire.
...
Table~\ref{tab:results_confs} summarizes performance across the test set, reporting the count of instances where the best ($B$) and median ($M$) solutions over 5 runs matched or improved the BKS.
...
Across the benchmark suite, $C_1$ matches or improves the BKS in 72.2\% of instances, maintaining high competitiveness across all scales: 70.8\% for $n=10$, 75\% for $n=15$, and 70.8\% for $n=20$.
Analysis of median results further confirms algorithmic robustness: across 5 stochastic runs, the median solution matched or improved upon the literature best in 48.6\% of instances, with an average percentage deviation of only 0.96\% relative to the best individual runs.
...
\input{table_wilcoxon.tex}

Table~\ref{tab:comparacao_estado_wilcoxon} summarizes the pairwise comparison and one-tailed Wilcoxon signed-rank tests against each baseline. Across the complete 72 instances, $C_1$ matches or improves upon \citet{dos2010combination} (94.4\%), \citet{wang2014variable} (97.2\%), and \citet{ahmadian2021meta} (77.7\%), with statistically significant differences ($p < 0.05$).
...
\input{table_new_upper_bounds.tex}
